...
namespace TextSplit
{
    ...
    class TextBookAnalysis : ITextBookAnalysis
    {
        ...
        {   
            ...
            if (desiredTextLanguage < 0) return desiredTextLanguage;//типа, нечего анализировать
            ...
            {//если первоначальный анализ текста, то делим текст на абзацы
                ...
                if (portionBookTextResult > 0)
                {
                    _book.SetFileToDo((int)WhatNeedDoWithFiles.SelectChapterName, desiredTextLanguage);
                ...
                //if (portionBookTextResult > 0)
                //{
                //    int i = CurrentParagraphsPortioned(desiredTextLanguage);
                //}

                return portionBookTextResult;
            }
            if (_book.GetFileToDo(desiredTextLanguage) == (int)WhatNeedDoWithFiles.AnalyseChapterName)
            {//если уже выбран текст названия главы, то изучаем полученный выбранный текст
                int chapterCount = SelectedChapterNameAnalysis(desiredTextLanguage);
        ...
        private int PortionBookTextOnParagraphs(int desiredTextLanguage)//делит текст на абзацы по EOL, сохраняет в List в AllBookData
        {
            ...
            for (int i = 0; i < textOnParagraphsPortionedLength; i++)
            {
                _messageService.ShowTrace(MethodBase.GetCurrentMethod().ToString(), strCRLF + "TextOnParagraphsPortioned [i], where i = " + i.ToString() + strCRLF +
                    TextOnParagraphsPortioned[i], CurrentClassName, showMessagesLevel);

                addParagraphTextCount = _book.AddParagraphText(TextOnParagraphsPortioned[i], desiredTextLanguage);//возвращает количество элементов
